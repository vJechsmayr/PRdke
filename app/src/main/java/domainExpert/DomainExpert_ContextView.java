 ...
 * @author Viktoria J.
 * 
 *         Liste Context OK Anfrage new Context -> RepAdmin OK Anfrage update
 *         Context -> RepAdmin (nice to have) - Anfrage delete Context ->
 *         RepAdmin OK
 * 
...
public class DomainExpert_ContextView extends DomainExpertViews implements View {
			...
			deleteCtx.addClickListener(new Button.ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					...
					if (select.getSelectedItem().isPresent()) {
						OperationPosition op = SystemHelper.isComposedOperationsStarted(SystemHelper.COM_DELETE_CONTEXT,
					...
					if (select.getSelectedItem().isPresent()) {
						if (contextName.getValue() != null && contextName.getValue() != "") {
							...
							OperationPosition op = SystemHelper.isComposedOperationsStarted(
									SystemHelper.COM_NEW_CONTEXT, "", "", contextName.getValue());
	 ...
	 * @author Marcel G.
	 * 
	 * edit by Viktoria J. Build list of children or parents of given context
	 */
